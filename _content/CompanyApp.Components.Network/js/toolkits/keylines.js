/**
 * KeyLines Network Visualization Wrapper
 * Provides integration between Blazor and KeyLines SDK
 * Falls back to D3.js if KeyLines is not available
 *
 * DEPENDENCIES:
 * - KeyLines SDK (commercial license required)
 * OR
 * - D3.js v7+ as fallback (https://d3js.org/)
 */

class KeyLinesNetworkGraph {
    constructor(containerId, options = {}) {
        ...
        this.nodes = [];
        this.links = [];
        this.isKeyLinesAvailable = typeof KeyLines !== 'undefined';

        console.log(`KeyLines available: ${this.isKeyLinesAvailable}`);

        this.init();
    }

    init() {
        if (this.isKeyLinesAvailable) {
            this.initKeyLines();
        } else {
            console.warn('KeyLines not available, falling back to D3.js');
            this.initD3Fallback();
        }
    }
    ...
    initKeyLines() {
        try {
            // Initialize KeyLines chart
            KeyLines.create(this.containerId, {
                width: this.options.width,
                height: this.options.height,
                type: 'graph'
            }, (err, chart) => {
                if (err) {
                    console.error('KeyLines initialization error:', err);
                    this.initD3Fallback();
                    return;
                }
                ...
                console.log('KeyLines chart initialized');
            });
        } catch (error) {
            console.error('Failed to initialize KeyLines:', error);
            this.initD3Fallback();
        }
    }
    ...
    initD3Fallback() {
        if (typeof d3 === 'undefined') {
            console.error('Neither KeyLines nor D3.js is available. Please include one of them.');
    ...
    loadData(data) {
        ...
        if (this.isKeyLinesAvailable && this.chart) {
            this.loadDataKeyLines(data);
        } else if (this.svg) {
            this.loadDataD3(data);
        }
    }

    loadDataKeyLines(data) {
        try {
            // Convert data to KeyLines format
            const items = {
                type: 'LinkChart',
                items: [
                    ...this.nodes.map(node => ({
            ...
            console.log(`Loaded ${this.nodes.length} nodes and ${this.links.length} links into KeyLines`);
        } catch (error) {
            console.error('Error loading data into KeyLines:', error);
        }
    }
        ...
        this.container.innerHTML = `
            <div style="display: flex; align-items: center; justify-content: center; height: 100%; text-align: center;">
                <div>
                    ...
                    <p>Please include either KeyLines SDK or D3.js library.</p>
    ...
    destroy() {
        if (this.isKeyLinesAvailable && this.chart) {
            try {
                ...
                console.error('Error destroying KeyLines chart:', e);
            }
        } else if (this.simulation) {
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = KeyLinesNetworkGraph;
}
...
export function initializeNetworkGraph(containerId, options, dotNetReference) {
    try {
        ...
        const graph = new KeyLinesNetworkGraph(containerId, options);
